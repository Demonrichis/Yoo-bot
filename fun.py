# D.S.O BOT — Fun Command System v1.1 (Full Fixed)
# ------------------------------------------------------------------------------------
# What’s new in 1.1 (compared to your previous file):
#   • FIXED: All button callbacks now use the correct signature:
#       async def handler(self, interaction: discord.Interaction, button: discord.ui.Button)
#     (This removes the AttributeError: 'Button' object has no attribute 'user')
#   • FIXED: Same signature correction in Paginator + SettingsView.
#   • Kept: defer + followup pattern to avoid “Interaction Failed”.
#   • Kept: Big reaction emoji pool (80+), favorites, stats, aliases, settings, paginator.
#
# Requirements (Python 3.10+):
#   pip install -U discord.py aiohttp
#
# Replace the two tokens below before running.
# ------------------------------------------------------------------------------------
...
EMBED_FOOTER      = "Version 1.1 • Developer: DEMON • Powered by DEMON'S SERVER"
...
async def fetch_tenor_gifs(query: str, limit: int = 15) -> List[str]:
    """Fetch up to `limit` GIF URLs from Tenor v2."""
    ...
    for item in data.get("results", []):
        ...
        gif_obj = media.get("gif")
        if gif_obj and gif_obj.get("url"):
            urls.append(gif_obj["url"])
...
async def ensure_action_gifs_cached(action: str, force_min: int = 1) -> None:
    """Ensure we have at least `force_min` GIFs cached for an action (fetches ~15 if low)."""
    entry = actions_store.setdefault(action, {"gifs": [], "aliases": []})
    if len(entry["gifs"]) >= force_min:
        return
    urls = await fetch_tenor_gifs(f"anime {action} gif", limit=15)
    if urls:
        entry["gifs"] = (urls + entry["gifs"])[:15]
        save_json(ACTIONS_FILE, actions_store)
...
# ------------------------------------------------------------------------------------
# Paginator View (FIXED signatures)
# ------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------
# Action View (Buttons under action embeds) — FIXED signatures
# ------------------------------------------------------------------------------------
class ActionView(discord.ui.View):
    ...
    async def react_back(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title=f"{target.display_name} {self.action}s {author.display_name}",
    ...
    async def repeat(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title=f"{author.display_name} {self.action}s {target.display_name}",
            color=discord.Color.random(),
        )
...
async def send_action_embed(channel: discord.abc.Messageable,
                            author: discord.Member,
                            target: discord.Member,
                            action: str,
                            auto_record_stats: bool = True):
    await ensure_action_gifs_cached(action, force_min=1)
    gifs = actions_store.get(action, {}).get("gifs", [])
    gif = random.choice(gifs) if gifs else None

    title = f"{author.display_name} {action}s {target.display_name}"
...
# ------------------------------------------------------------------------------------
# Single on_message handler (no duplicates)
# ------------------------------------------------------------------------------------
@bot.event
async def on_message(message: discord.Message):
    ...
    if m_add:
        ...
        urls = await fetch_tenor_gifs(f"anime {action_name} gif", limit=15)
        if not urls:
    ...
    if m_favu:
        ...
        embed = discord.Embed(title=f"{message.author.display_name} {canon}s …", color=discord.Color.random())
    ...
    for i, part in enumerate(parts):
        ...
        await send_action_embed(message.channel, message.author, target_member, action)
...
async def on_ready():
    ...
    print("Fun Command System v1.1 ready.")
...
if __name__ == "__main__":
    if DISCORD_BOT_TOKEN == "YOUR_DISCORD_BOT_TOKEN" or TENOR_API_KEY == "YOUR_TENOR_API_KEY":
        raise SystemExit("Set DISCORD_BOT_TOKEN and TENOR_API_KEY in the script.")
